...
Shader& Assets::getShader(const std::string& name)
{
	if ( shaders.find(name) == end(shaders))
	{
        ...
        loadError << "Shader " << name << " does not exist ine assets manager.";
...
{
	// 1.Retrieve the vertex/fragment source code filePath
    ...
    try
    {
        ...
        if (teShaderFile != "")
        {
            std::ifstream tessEvalShaderFile(tcShaderFile);
            std::stringstream teShaderStream;
            teShaderStream << tessEvalShaderFile.rdbuf();
            tessEvalShaderFile.close();
            tcCode = teShaderStream.str();
        }
        // If geometryt shader path is present, also load a geometryl shader
        if (tcShaderFile != "")
        {
            std::ifstream geometryShaderFile(tcShaderFile);
            std::stringstream gShaderStream;
            gShaderStream << geometryShaderFile.rdbuf();
            geometryShaderFile.close();
            tcCode = gShaderStream.str();
        }
    }
