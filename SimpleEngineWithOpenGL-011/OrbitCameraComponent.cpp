...
void OrbitCameraComponent::update(float dt)
{
	...
	Vector3 camereraPosition = target + offset;
	Matrix4 view = Matrix4::createLookAt(camereraPosition,target, up);
	setViewMatrix(view);
}

void OrbitCameraComponent::setPitchSpeec(float pitchSpeedP)
{
