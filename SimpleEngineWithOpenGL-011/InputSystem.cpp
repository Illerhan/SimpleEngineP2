...
bool InputSystem::initialize()
{
	...
	memset(inputState.keyboard.previousState, 0, SDL_CONTROLLER_BUTTON_MAX);
...
void InputSystem::update()
{
	...
	else inputState.mouse.currentButtons = SDL_GetMouseState(&x, &y);
	...
	inputState.controller.rightTrigger = filter1D(SDL_GameControllerGetAxis(controller, SDL_CONTROLLER_AXIS_TRIGGERLEFT));
...
float InputSystem::filter1D(int input)
{
	...
	{
		//Compute fractional value beteween dead zone and max value
...
Vector2 InputSystem::filter2D(int inputX, int inputY)
{
	...
	{
		// Calculate fractional value between deadzone and max value circles
		float f = (length - deadZone) / (maxValue - deadZone);
		// Clamp f between 0.0f adn 1.0f
